...
Sprite atlas: ../hojek-atlas.png, 8x11 grid of 192x208 cells. Look rows 9-10
were never generated by the hatch pipeline, so there is no cursor-gaze."""
...
class HachikoUsage(QWidget):
    ...
    def _load_frames(self):
        atlas = QPixmap(str(ATLAS))
        if atlas.isNull():
            raise SystemExit(f"cannot load atlas: {ATLAS}")
        w = round(CW * self.scale)
        h = round(CH * self.scale)
        for name, s in STATES.items():
            self.frames[name] = [
                atlas.copy(QRect(i * CW, s["row"] * CH, CW, CH)).scaled(
                    w, h,
